...
export const Battery = ({ percent = 100, size = 24, charging = false, outlined = false, color, ...props }) => {console.log(outlined); return (
  <svg height={`${size}px`} width={`${size}px`} x="0px" y="0px" viewBox="0 0 24 24" {...props}>
    <defs>
      ...
      <mask id="test">
        <rect width="100%" height="100%" fill="white" />
        <rect x="2" y="7" fill="black" width={`${Math.min(percent * 18 / 100, 18)}`} height="10" />
      </mask>
    </defs>
    ...
    {percent > 0 && 
      <g fill={color || "currentColor"} >
        <rect x="2" y="7" clipPath={`url(#${getID(charging)})`} width={`${Math.min(percent * 18 / 100, 18)}`} height="10" mask={(outlined && charging) ? "url(#hole)" : null} />
        {outlined && charging && <polygon points="11.5,13 11.5,15 4,11 9.5,11 9.5,9 17,13" mask="url(#test)" />}
      </g>
    }
